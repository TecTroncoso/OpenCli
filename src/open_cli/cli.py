...
from open_cli.constant import VERSION
...
cli = typer.Typer(
    ...
    help="OpenCLI, your next CLI agent.",
)
...
def _version_callback(value: bool) -> None:
    if value:
        typer.echo(f"opencli, version {VERSION}")
        raise typer.Exit()
...
def opencli(
    ...
    """OpenCLI, your next CLI agent."""
    del version  # handled in the callback

    from open_cli.app import OpenCLI
    from open_cli.session import Session
    from open_cli.share import get_share_dir
    from open_cli.utils.logging import logger
    ...
    logger.add(
        get_share_dir() / "logs" / "opencli.log",
